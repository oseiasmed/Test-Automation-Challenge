module.exports = {
    ...
    scrolling: async function (page, selector) {
        try {
            ...
            await page.mouse.wheel({ deltaX: 25000 })

        } catch (error) {
            throw new Error(`Could not scrolling }`)
        }
    },
    ...
    especialClick: async function (page, selector, ...ops) {
        try {

            var selectInput = await page.$x(selector)
            await selectInput[0].click(selector)

            var selectInput2 = await page.$x(ops)
            while (ops) {
                await selectInput2[0].click(ops)
            }
        } catch (error) {
            throw new Error(`Could not click in a option: ${ops}`)
        }
    },
