...
describe('Must do a crud on Guarantee items', () => {
	...
	it('Must delete a Guarantee', async () => {
		...
		const guarIn_Del_MSG = await page.evaluate(() => {
			const guarIn_Del__Ok = document.querySelector('.ant-message').textContent
            return guarIn_Del__Ok
		...
		if (guarIn_Del_MSG) {
			...
			expect(guarIn_Del_MSG).toBe('Não é possível excluir essa garantia, pois ela já possui movimentações')
